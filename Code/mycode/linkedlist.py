...
class LinkedList(object):
    ...
    def get_at_index(self, index):
        ...
        Best case running time: Omega(1) item is the first in list
        Worst case running time: O(n) item is last in list"""
    ...
    def replace(self, old_item, new_item):
        ...
        Best case running time: Omega(1) replace item is first in list
        Worst case running time: O(n) item is last in list"""
        # TODO: Find the node containing the given old_item and replace its
        # data with new_item, without creating a new node object
        node = self.head
        while node is not None and node.data is not old_item:
            node = node.next
        if node is not None:
            node.data = new_item
        else:
    ...
    def delete(self, item):
        ...
        Best case running time: Omega(1) deletable item is first in list
        Worst case running time: O(n) deletable item is last in the list or not in list"""
        # Start at the head node
        node = self.head
